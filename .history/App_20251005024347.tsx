...
*/}
  ...
  return (
    <div className="relative flex flex-col h-screen font-sans text-slate-200">
      ...
      {showNearestHospital && nearestHospitals.length > 0 && (
        <div
          style={{
            position: "fixed",
            top: "10px",
          ...
          {nearestHospitals.map((entry, idx) => {
            ...
            return (
              <div key={entry.hospital.id} style={{ marginBottom: "0.5rem" }}>
                Nearest Hospital {idx + 1}: {entry.hospital.name} — Wait Time: {waitTime ?? "N/A"} minutes
                {waitTime != null && waitTime !== "N/A" ? null : null}
              </div>
            );
          }).find(hospitalDiv => {
            // Only show hospitals until a valid wait time is found
            const waitTime = hospitalDiv.props.children[2];
            return waitTime !== "N/A";
          }) || nearestHospitals.map((entry, idx) => (
            <div key={entry.hospital.id} style={{ marginBottom: "0.5rem" }}>
              Nearest Hospital {idx + 1}: {entry.hospital.name} — Wait Time: {entry.hospital.waitTime?.waitTimeMinutes ?? "N/A"} minutes
            </div>
          ))}
        </div>
      )}
      ...
      {/* Hamburger Menu (single instance) */}
      <div style={{
        position: "fixed",
        top: "10px",
        left: "10px",
            ...
            nearestHospitals.map((entry, idx) => {
              ...
              return (
                <div key={entry.hospital.id} style={{ marginBottom: "1rem", fontSize: "1.1rem" }}>
                  <strong>{entry.hospital.name}</strong><br />
                  Wait Time: {waitTime ?? "N/A"} minutes
                </div>
              );
            })
          ) : (
