...
const App: React.FC = () => {
  ...
  const nearestHospital = userLocation && waitTimes.length > 0
  ? waitTimes.reduce((nearest, hospital) => {
  ...
  return (
    <div className="relative flex flex-col h-screen font-sans text-slate-200">
      ...
      {showNearestHospital && nearestHospital && (
        ...
        >
          Nearest Hospital: {nearestHospital.hospital.name} — Wait Time: {nearestHospital.hospital.waitTime?.waitTimeMinutes ?? "N/A"} minutes
        </div>
      )}
      ...
      <button onClick={requestUserLocation}>Show Nearest Hospital</button>  <button onClick={requestUserLocation}>Show Nearest Hospital</button>
    </div>  </div>
  );  );
};
