  ...
  return (
    <div className="relative flex flex-col h-screen font-sans text-slate-200">
      ...
      {showNearestHospital && nearest && (
        ...
        >
          Nearest Hospital: {nearest.hospital.name} — Wait Time: {nearest.hospital.waitTime?.waitTimeMinutes ?? "N/A"} minutes
          {nearest.hospital.waitTime?.waitTimeMinutes == null && secondNearest && (
            <div style={{ marginTop: "0.5rem" }}>
              Second Nearest: {secondNearest.hospital.name} — Wait Time: {secondNearest.hospital.waitTime?.waitTimeMinutes ?? "N/A"} minutes
            </div>
          )}
        </div>
      )}
